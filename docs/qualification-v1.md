...
GitHub-hosted runner. A
trusted operator must set the repository secret `LAYA_CI_BUNDLE_URL` to an
HTTPS URL for a root-layout bundle archive, then provide that archive's SHA-256
as the workflow input. The URL may be short-lived and authenticated by a query
token; it is never supplied as a public workflow input or printed by the job.
...
It verifies archive digests before
